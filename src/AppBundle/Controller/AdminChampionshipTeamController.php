...
class AdminChampionshipTeamController extends AdminController
{
    /**
     * @Route("/admin/championships/{id}/teams/", name="admin_ct_team_route", requirements={"id": "\d+"})
     * @Method({"GET", "POST"})
    ...
    public function teamAction(Request $request, $id)
    {
        ...
        $this->page = 'championship';
        $this->action = 'team';

        // execute data table actions
        $this->dataTableAction($request, 'AppBundle:ChampionshipTeam');
        ...
        $qParams['data']['criteria']['championship'] = $id;

        list($teams, $total) = $this->getDoctrine()->getRepository('AppBundle:ChampionshipTeam')->findByLike(
        ...
        $parameters = [
            'page_route' => 'admin_ct_team_route',
        ];

        return $this->render($request->isXMLHttpRequest() ? 'dataTable/table.html.twig' : 'championship/teams.html.twig', $parameters);
    }

    /**
     * @Route("/admin/teams/{id}/championships/", name="admin_ct_championship_route", requirements={"id": "\d+"})
     * @Method({"GET", "POST"})
     * @Security("has_role('ROLE_ADMIN')")
     */
    public function championshipAction(Request $request, $id)
    {
        ...
        list($championships, $total) = $this->getDoctrine()->getRepository('AppBundle:ChampionshipTeam')->findByLike(
            $qParams['data']['criteria'],
            $qParams['data']['orderBy'],
            $qParams['data']['limit'],
            $qParams['data']['offset']
    ...
    protected function initDataTable(Request $request, $id){
        $route = $this->action == 'team' ? 'admin_ct_team_route' : 'admin_ct_championship_route';
        // data table
        ...
        if($this->action == 'team'){
            ...
            $this->dataTable->addSearchForm(['c.name']);
        }


        // add table columns
        $route = $this->action == 'team' ? 'admin_detail_team_route' : 'admin_detail_championship_route';
